...
```bash
curl https://api.example.com/api/productos/aB3dE7fGhJ \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X PATCH https://api.example.com/api/productos/aB3dE7fGhJ \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE https://api.example.com/api/productos/aB3dE7fGhJ \
  -H "Authorization: Bearer <token>"
```
