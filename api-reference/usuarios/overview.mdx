...
```bash
curl https://api.example.com/api/usuarios/usr_abc123def456ghi7 \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X PUT https://api.example.com/api/usuarios/usr_abc123def456ghi7 \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"rol": "admin"}'
```
...
```bash
curl -X DELETE https://api.example.com/api/usuarios/usr_abc123def456ghi7 \
  -H "Authorization: Bearer <token>"
```
